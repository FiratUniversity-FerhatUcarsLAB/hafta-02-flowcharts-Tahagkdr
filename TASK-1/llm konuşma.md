graphviz e göre bana atm den para çekme kodunu yazarmısın
ChatGPT:

Harika — ATM’den para çekme işlem akışını Graphviz (DOT) diliyle bir akış diyagramı olarak hazırladım. Aşağıdaki kodu bir .dot dosyasına yapıştırıp Graphviz ile render edebilirsiniz (dot -Tpng -o atm.png atm.dot gibi).

Kod hem normal akışı hem de hata/iptal dallarını (yanlış PIN, yetersiz bakiye, zaman aşımı, kart tutulması) içerir.
...
Kodun tamamını bir dosyaya yapıştırın, örn atm.dot.

Komut satırında Graphviz kuruluysa:
dot -Tpng -o atm.png atm.dot
veya SVG için: dot -Tsvg -o atm.svg atm.dot

Oluşan görsel akış diyagramını açın.

İstersen:

Diyagramı Türkçe terimler yerine İngilizceye çeviririm,

Daha sade bir versiyon (yalnızca temel akış) oluştururum,

Ya da her düğüm için açıklama ekleyip bir README ile birlikte .dot + .png paketleyebilirim.

Hangi varyasyonu istersin?
